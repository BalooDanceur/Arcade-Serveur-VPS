...
mergePP=function mergePP(entry,pp){
var ppUsed=entry[1];
...
var pp=callerMoveForPressure?0:1;
...
pp=this.getPressurePP(pp,foeTargets.filter(function(foe){return foe&&!foe.fainted;}));
}
...
getPressurePP=function getPressurePP(pp,foes){for(var _i36=0;_i36<
foes.length;_i36++){var foe=foes[_i36];
...
if(nextArgs[3]==='Moustillonite'){
newSpeciesForme='Oshawott-Mega';
species=this.dex.species.get('Oshawott-Mega');
}else if(_item3.megaStone){
...
if(_poke32 && !(_effect15.id==='typechange'&&_poke32.terastallized)&&
_effect15.id!=='futuresight'&&_effect15.id!=='doomdesire'){
_poke32.addVolatile(_effect15.id);
}
if(_poke32)this.scene.updateStatbar(_poke32);
this.log(args,kwArgs);
break;
}
case'-end':{
